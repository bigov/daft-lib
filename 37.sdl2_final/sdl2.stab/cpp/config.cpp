...
namespace app {
	...
	//
	void Config::detect_os() throw(errCfg)
	{
		...
		if(NULL == w) throw errCfg("");
    if(NULL == SDL_GL_CreateContext(w)) throw errCfg("Can't create context");
    gl::exts::LoadTest didLoad = gl::sys::LoadFunctions();
    if(NULL == didLoad) throw errCfg("Can't load OpenGL functions");
    ...
    gl::GetIntegerv(gl::MINOR_VERSION, &minor);
		...
		SDL_DestroyWindow(w);
		if(!rez) throw errCfg("Can't determinate OS!");
	...
	/** Считывание данных из файла настроек
		...
		*/
	void Config::read_ini_file() throw(errCfg)
	{
		...
		delete config_file;
		if(!ini) throw errCfg("can't read " + *config_file);
	...
	void Config::get_ini_config() throw(errCfg)
	{
	...
	Config::Config(int argc, char * argv[]) throw(errCfg):
		ini_cfg(), run_dir(nullptr), sys_dir(nullptr), ds(), major(3),
		minor(0), server(), gui(), content() 
	{
