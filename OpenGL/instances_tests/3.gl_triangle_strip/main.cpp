...
// file: main.cpp
//
// Общий буфер атрибутов + индексный буфер по методу GL_TRIANGLE_STRIP
//
//----------------------------------------------------------------------------
#include "tools.hpp"
#include "texture.hpp"

// VBO массив из 4 точек для создания прямоугольника из 2-х треугольников:
...
void start_application(void)
{
  ...
  float time = 0.f, dTime = 0.f;
    ...
    glUniformMatrix4fv(uniMvp, 1, GL_FALSE, glm::value_ptr(proj * view * model));
		...
		glDrawElements(GL_TRIANGLE_STRIP, render_points, GL_UNSIGNED_INT, NULL);
