...
class World(object):
    ...
    # массив координат точек элементов 3D пространства
    space = None
    ...
    def gen_space(self, location):
        self.space = self.square_3d(location, 100)

        material = self.tex_coords((0,2), (0,2), (0,2), 1)

        # Установить пирамиду видимости
        self.set_frustrum()

        for point in self.space:
            if self.PointInFrustum(*point):
                self.draw_squares(point, material)

    def draw_squares(self, point, texture):
        plates = self.gen_plate(*point)

        for i in range(len(plates)):
            obj_data = (('v3f/static', plates[i]),
                    ('t2f/static', texture[i]),
                    ('n3f', self.cube_normals[i]))
            self.batch.add(4, GL_QUADS, self.TexGroup, *obj_data)
