...
void win_gl::app_close(void)
{
  hide();
}

win_gl::win_gl() : m_RotationAngles(N_AXIS, 0.0f)
{
  set_title("GL Area");
  ...
  m_GLArea.signal_realize().connect(sigc::mem_fun(*this, &win_gl::realize));
  ...
  m_GLArea.signal_unrealize().connect(sigc::mem_fun(*this, &win_gl::unrealize), false);
  m_GLArea.signal_render().connect(sigc::mem_fun(*this, &win_gl::render), false);
  ...
  m_Button.signal_clicked().connect(sigc::mem_fun(*this, &win_gl::app_close));
  show_all();
}
...
void win_gl::on_axis_value_change(int axis, const Glib::RefPtr<Gtk::Adjustment>& adj)
{
...
bool win_gl::render(const Glib::RefPtr<Gdk::GLContext>& /* context */)
{
  try
  {
    m_GLArea.throw_if_error();
...
Gtk::Box* win_gl::create_axis_slider_box(int axis)
{
  ...
  adj->signal_value_changed().connect(
    sigc::bind(sigc::mem_fun(*this, &win_gl::on_axis_value_change), axis, adj)
                                      );
...
///
static GLuint
create_shader( unsigned int type, Glib::RefPtr<const Glib::Bytes> src )
{
  gsize size {src->get_size()};
  const GLchar* ch = static_cast<const GLchar*>(src->get_data(size));
...
void win_gl::init_shaders()
{
  ...
  if(status == GL_FALSE)
  {
    ...
    std::vector<GLchar> log_space(static_cast<unsigned int>(log_len+1), ' ');
    glGetProgramInfoLog(m_Program, log_len, nullptr, log_space.data());

    cerr << "Linking failure: " << log_space.data() << endl;
