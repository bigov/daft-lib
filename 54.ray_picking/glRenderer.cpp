...
{
	shader = loadShader("raypicking.vert", "raypicking.frag");
}
...
//
static int rayIntersectsTriangle(	glm::vec3 origin, glm::vec3 dir,
	glm::vec3 v0, glm::vec3 v1, glm::vec3 v2,	float* intersection )
{
	// Triangle edges
	glm::vec3 e1(v1 - v0);
	glm::vec3 e2(v2 - v0);
...
void glRenderer::mouseButtonCallback(int button, int action)
{
	...
	if ((action == GLFW_PRESS) && (button == GLFW_MOUSE_BUTTON_1))
	{
		...
		glm::vec3 worldNear = glm::unProject(
				glm::vec3(float(mx), float(h - my), 0.0f),
				uboVS.view * uboVS.model,
				uboVS.projection, viewport);
			
		// Mouse world pos on far plane
		glm::vec3 worldFar = glm::unProject(
				glm::vec3(float(mx), float(h - my), 1.0f),
				uboVS.view * uboVS.model,
				uboVS.projection, viewport);
		...
		for (uint32_t i = 0; i < indexCount / 3; i++)
		{
			...
			if( 
				rayIntersectsTriangle ( 
						worldNear, rayDir,
						triangleData[i*3], triangleData[i*3 + 1], triangleData[i*3 + 2],
						&currIntersectionPos
					)
				)
			{
